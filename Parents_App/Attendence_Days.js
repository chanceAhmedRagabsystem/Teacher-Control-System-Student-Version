...
import { Background, initial_color, beige_color, secondary_color, Café_color, gray_color } from '../Student_App/Colors'
...
export default class Attendence_Days extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            attendence: [
                        ...
                        {this.state.attendence.map((item, index) =>
                            item.show == 1 ? (
                                <View
                                    ...
                                    <Text style={{ color: '#1D1D1D', fontSize: 16, alignSelf: 'center', fontWeight: 'bold' }}>{this.state.attendence[0].text}</Text>
